"""FROZEN reference copy of the pre-provenance markdown chunker.

Verbatim body of langextract_docling/markdown_chunker.py at commit 7e53403
("Upgrade to langextract 1.6.0 and docling 2.111.0") — the last iteration
before the chunker was reimplemented over ProvenanceMarkdownSerializer. It
is the ground-truth side of the text invariant (test spec §6): the new
serializer's output must equal this chunker's chunks joined with "\\n\\n".

Do not modify except to re-freeze deliberately; only the docstring differs
