...
Every extraction traces back to the exact spot it came from in the source
PDF — down to the words on the page, or the single table cell that holds
them, not the whole table:

![Animated demo of PDF highlight visualization, stepping through extractions with boxes narrowed to the exact words or table cell they came from](examples/hero_demo.gif)

*Generated by [`examples/run_hero_pipeline.py`](examples/run_hero_pipeline.py); see [PDF highlight visualization](#pdf-highlight-visualization) below.*
